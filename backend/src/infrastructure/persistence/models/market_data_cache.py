...
from sqlalchemy import (
    BigInteger, Boolean, CheckConstraint, Column, Enum, Index, Integer,
...
class MarketDataCache(Base, DatabaseMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index('idx_market_data_cache_significant', 'is_significant_movement', 
...
class TimelineMetrics(Base, DatabaseMixin, TimestampMixin):
    ...
    last_calculated = Column(TIMESTAMP(timezone=True), nullable=False,
                           default=lambda: datetime.now(timezone.utc))
...
class DataGaps(Base, DatabaseMixin, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index('idx_data_gaps_unresolved', 'is_resolved', 
...
class PrecisionPolicies(Base, DatabaseMixin, TimestampMixin):
    ...
    priority = Column(Integer, nullable=False, default=100)  # Plus bas = plus prioritaire

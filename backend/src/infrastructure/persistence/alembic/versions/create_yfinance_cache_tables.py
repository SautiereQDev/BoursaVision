...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('data_source', sa.String(length=20), nullable=False, default='yfinance'),
        ...
        sa.Column('is_significant_movement', sa.Boolean(), nullable=False, default=False),
        ...
        sa.CheckConstraint(
            'cache_priority >= 1 AND cache_priority <= 10',
            name='check_cache_priority'
        ),
    )
    
    # Index optimisés pour TimescaleDB
    op.create_index(
        'idx_market_data_cache_symbol_time', 
        'market_data_cache', 
        ['symbol', 'time']
    )
    op.create_index(
        'idx_market_data_cache_time_desc', 
        'market_data_cache', 
        [sa.text('time DESC')]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column('data_coverage_percent', sa.Numeric(precision=5, scale=2), nullable=False, default=0),
        ...
        sa.Column('precision_distribution', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        ...
        sa.Column('data_quality_score', sa.Numeric(precision=5, scale=2), nullable=False, default=0),
        sa.Column('average_access_frequency', sa.Numeric(precision=8, scale=2), nullable=False, default=0),
        sa.Column('cache_hit_rate', sa.Numeric(precision=5, scale=2), nullable=False, default=0),
        sa.Column('last_calculated', postgresql.TIMESTAMP(timezone=True), nullable=False),
        sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), nullable=True),
        sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), nullable=True),
    ...
    op.create_index('idx_timeline_metrics_quality', 'timeline_metrics', ['data_quality_score'])
    op.create_index('idx_timeline_metrics_last_calc', 'timeline_metrics', ['last_calculated'])
    
    # Table des statistiques de cache
    op.create_table(
        'cache_statistics',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('measurement_start', postgresql.TIMESTAMP(timezone=True), nullable=False),
        sa.Column('measurement_end', postgresql.TIMESTAMP(timezone=True), nullable=False),
        ...
        sa.Column('average_response_time_ms', sa.Numeric(precision=8, scale=2), nullable=True),
    ...
    op.create_index('idx_cache_stats_period', 'cache_statistics', ['measurement_start', 'measurement_end'])
    ...
    op.create_table(
        ...
        sa.Column('interval_types', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        ...
        sa.Column('cache_priority_multiplier', sa.Numeric(precision=3, scale=2), nullable=False, default=1.0),
        sa.Column('enable_predictive_caching', sa.Boolean(), nullable=False, default=False),
        sa.Column('max_cache_points_per_symbol', sa.Integer(), nullable=False, default=10000),
    ...
    op.create_index('idx_precision_policies_active', 'precision_policies', ['is_active'])
    op.create_index('idx_precision_policies_priority', 'precision_policies', ['priority'])

...
class IMarketDataTimelineRepository(Protocol):
    ...
    async def get_latest_point(self, symbol: str, interval_type: IntervalType) -> Optional[TimelinePoint]:
...
class SqlAlchemyMarketDataTimelineRepository:
    ...
    def _db_to_timeline_point(self, db_point: MarketData) -> TimelinePoint:
        ...
        return TimelinePoint(
            ...
            source=DataSource(db_point.source) if db_point.source else DataSource.YFINANCE,
...
class MarketDataTimelineSpecification:
    ...
    def significant_movement(min_change_percent: float = 5.0):
        ...
        return func.abs(
            (MarketData.close_price - MarketData.open_price) / MarketData.open_price * 100
        ) >= min_change_percent
...
class RepositoryFactory:
    ...
    @staticmethod
    def create_timeline_repository(session: AsyncSession) -> IMarketDataTimelineRepository:

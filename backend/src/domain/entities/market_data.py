...
@dataclass
class MarketDataArgs:
    """Arguments for creating market data"""
...
class MarketData(AggregateRoot):
    ...
    @classmethod
    def create(cls, args: MarketDataArgs) -> "MarketData":
        ...
        market_data = cls(
            ...
            volume=args.volume,
            adjusted_close=args.adjusted_close,
            dividend_amount=args.dividend_amount,
            split_coefficient=args.split_coefficient,
        )

        market_data._add_domain_event(
    ...
    def update_prices(self, **prices):
        """Update prices using keyword arguments to reduce the number of parameters."""
        self.open_price = prices.get("open_price", self.open_price)
        self.high_price = prices.get("high_price", self.high_price)
        self.low_price = prices.get("low_price", self.low_price)
    ...
    def is_gap_up(self, threshold_percent: Decimal = Decimal("2")) -> bool:
        ...
        if self.adjusted_close is None:
            return False
        gap_percent = (
            (self.open_price - self.adjusted_close) / self.adjusted_close
        ) * Decimal("100")
    ...
    def is_gap_down(self, threshold_percent: Decimal = Decimal("2")) -> bool:
        ...
        if self.adjusted_close is None:
            return False
        gap_percent = (
            (self.adjusted_close - self.open_price) / self.adjusted_close
        ) * Decimal("100")
        return gap_percent >= threshold_percent
    ...
    def has_split(self) -> bool:
        ...
        return self.split_coefficient is not None and self.split_coefficient != Decimal(
            "1"
        )

    def __str__(self) -> str:
        return (
            f"MarketData({self.symbol}, {self.timestamp.date()}, ${self.close_price})"
        )

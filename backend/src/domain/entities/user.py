    ...
    def permissions(self) -> List[str]:
        ...
        permissions_map = {
            self.ADMIN: [
                ...
                "create_portfolio", "delete_portfolio", "view_all_portfolios",
                "execute_trades", "view_analytics", "manage_alerts"
            ],
            self.TRADER: [
                "create_portfolio", "manage_own_portfolios",
                "execute_trades", "view_analytics", "manage_alerts"
            ],
            self.ANALYST: [
                "view_portfolios", "view_analytics", "create_reports"
            ],
            self.VIEWER: [
                "view_portfolios", "view_basic_analytics"
            ]
        }
...
class User(AggregateRoot):
    ...
    def __post_init__(self):
        ...
        if hasattr(self, '_is_new_user') and self._is_new_user:
            self._add_domain_event(UserCreatedEvent(
                user_id=self.id,
                email=self.email,

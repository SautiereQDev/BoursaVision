...
class TimelinePoint:
    ...
    def __post_init__(self):
        ...
        if self.high_price.amount < max(self.open_price.amount, self.close_price.amount):
    ...
    def price_change_percent(self) -> float:
        ...
        if self.open_price.amount == 0:
            return 0.0
        return float(((self.close_price.amount - self.open_price.amount) / self.open_price.amount) * 100)
    
    @property
    def typical_price(self) -> Money:
        """Prix typique (HLC/3)"""
        typical = (self.high_price.amount + self.low_price.amount + self.close_price.amount) / 3
...
class MarketDataTimeline(AggregateRoot):
    ...
    def add_point(self, point: TimelinePoint) -> None:
        ...
        if point.open_price.currency != self.currency:
            raise ValueError(f"Currency mismatch: expected {self.currency}, got {point.open_price.currency}")
        ...
        if point.timestamp in self._points:
            ...
            should_update = (
                (existing.source == DataSource.INTERNAL_CACHE and point.source != DataSource.INTERNAL_CACHE) or
                (point.precision_level.value < existing.precision_level.value)
            )
            if should_update:
    ...
    def add_points(self, points: List[TimelinePoint]) -> None:
        ...
        for point in points:
            ...
            if point.timestamp in self._points:
                existing = self._points[point.timestamp]
                if (existing.source == DataSource.INTERNAL_CACHE and point.source != DataSource.INTERNAL_CACHE) or \
                   (point.precision_level.value < existing.precision_level.value):
                    self._points[point.timestamp] = point
                    self._is_dirty = True
            else:
    ...
    def needs_refresh(
        ...
        interval_points = [p for p in self._points.values() if p.interval_type == interval_type]
    ...
    def get_metrics(self) -> TimelineMetrics:
        ...
        if self.latest_point and self.oldest_point:
            total_timespan = (self.latest_point.timestamp - self.oldest_point.timestamp).total_seconds()
    ...
    def _determine_expected_interval(
        self, 

...
class MarketDataCacheManager:
    ...
    def _generate_key(self, symbol: str, timestamp: datetime, interval: IntervalType) -> str:
        """Génère une clé de cache"""
        return f"{symbol}:{interval.value}:{timestamp.isoformat()}"
    
    def get(self, symbol: str, timestamp: datetime, interval: IntervalType) -> Optional[TimelinePoint]:
    ...
    def _evict_expired(self) -> None:
        ...
        for symbol in self._cache.keys():
            symbol_cache = self._cache[symbol]
            expired_timestamps = [
                ts for ts, entry in symbol_cache.items()
                if entry.is_expired
            ]
    ...
    def get_symbol_cache_info(self, symbol: str) -> Dict[str, Any]:
        ...
        avg_age = sum(
            (now - entry.cached_at).total_seconds() 
        ...
        return {
            ...
            'oldest_entry': min(
                (entry.cached_at for entry in symbol_cache.values()),
                default=None
            ),
            'newest_entry': max(
                (entry.cached_at for entry in symbol_cache.values()),

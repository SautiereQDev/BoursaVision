    ...
    def get_insights(
        ...
        try:
            ...
            if len(close_prices) >= 14:
                ...
                if current_rsi < 30:
                    insights.append(
                        f"RSI ({current_rsi:.1f}) indicates oversold conditions - potential buying opportunity"
                    )
                elif current_rsi > 70:
                    insights.append(
                        f"RSI ({current_rsi:.1f}) indicates overbought conditions - consider taking profits"
                    )
            ...
            if len(close_prices) >= 50:
                ...
                if current_price > sma_20 > sma_50:
                    ...
                    insights.append(
                        "Price is below key moving averages - bearish trend"
                    )
            ...
            if len(volumes) >= 20:
                ...
                if volume_ratio > 2.0:
                    insights.append(
                        "Exceptionally high volume detected - significant market interest"
                    )
    ...
    def get_insights(
        ...
        try:
            ...
            if pe_ratio:
                if pe_ratio < 15:
                    insights.append(
                        f"Low P/E ratio ({pe_ratio:.1f}) suggests potentially undervalued stock"
                    )
                elif pe_ratio > 30:
                    insights.append(
                        f"High P/E ratio ({pe_ratio:.1f}) indicates high growth expectations or overvaluation"
                    )
            ...
            if roe:
                ...
                if roe_percent > 15:
                    insights.append(
                        f"Strong ROE ({roe_percent:.1f}%) indicates efficient use of shareholder equity"
                    )
                elif roe_percent < 10:
                    insights.append(
                        f"Low ROE ({roe_percent:.1f}%) may indicate operational inefficiencies"
                    )
            ...
            if revenue_growth:
                ...
                if growth_percent > 10:
                    insights.append(
                        f"Strong revenue growth ({growth_percent:.1f}%) shows business expansion"
                    )
                elif growth_percent < 0:
                    insights.append(
                        f"Negative revenue growth ({growth_percent:.1f}%) indicates declining business"
                    )
            ...
            if debt_to_equity:
                if debt_to_equity < 0.5:
                    insights.append(
                        f"Low debt-to-equity ({debt_to_equity:.2f}) indicates strong financial position"
                    )
                elif debt_to_equity > 1.5:
                    insights.append(
                        f"High debt-to-equity ({debt_to_equity:.2f}) may indicate financial risk"
                    )
            ...
            if dividend_yield:
                ...
                if yield_percent > 4:
                    insights.append(
                        f"Attractive dividend yield ({yield_percent:.1f}%) for income investors"
                    )
        ...
        try:
            ...
            for period_name, days in periods:
                if len(close_prices) > days:
                    ...
                    if return_pct > 10:
                        insights.append(
                            f"Strong {period_name} momentum: +{return_pct:.1f}%"
                        )
                    elif return_pct < -10:
                        insights.append(
                            f"Negative {period_name} momentum: {return_pct:.1f}%"
                        )
            ...
            if len(close_prices) >= 20:
                ...
                if positive_ratio >= 0.7:
                    insights.append(
                        f"Consistent uptrend: {positive_ratio:.1%} positive days in last 20 sessions"
                    )
                elif positive_ratio <= 0.3:
                    insights.append(
                        f"Consistent downtrend: {positive_ratio:.1%} positive days in last 20 sessions"
                    )
...
class AdvancedInvestmentAnalyzer:
    ...
    def analyze_investment(self, symbol: str) -> ComprehensiveAnalysisResult:
        ...
        try:
            ...
            market_data = {"info": info, "history": hist}

            # Calculate scores for each strategy
            technical_score = self.technical_strategy.calculate_score(None, market_data)
            fundamental_score = self.fundamental_strategy.calculate_score(
                None, market_data
            )
    ...
    def _determine_recommendation(
        self, overall_score: float, technical_score: float, fundamental_score: float
    ) -> str:

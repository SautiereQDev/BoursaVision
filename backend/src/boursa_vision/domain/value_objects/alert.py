...
class AlertPriority(str, Enum):
    ...
    def weight(self) -> int:
        ...
        weights = {self.LOW: 1, self.MEDIUM: 2, self.HIGH: 3, self.CRITICAL: 4}
    ...
    def get_trigger_message(self, current_value: Decimal) -> str:
        ...
        condition_messages = {
            ...
            AlertCondition.LESS_THAN_OR_EQUAL: (
                f"{self.symbol} price ${current_value} has fallen to or below "
                f"${self.target_value}"
            ),
            AlertCondition.BETWEEN: (
                f"{self.symbol} price ${current_value} is within target range "
                f"${self.min_value}-${self.max_value}"
            ),
            AlertCondition.OUTSIDE_RANGE: (
                f"{self.symbol} price ${current_value} is outside target range "
                f"${self.min_value}-${self.max_value}"
            ),
        }

        return condition_messages.get(
            self.condition, f"{self.symbol} alert triggered: ${current_value}"
        )

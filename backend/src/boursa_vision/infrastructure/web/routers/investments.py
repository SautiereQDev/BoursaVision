...
class MoneyResponse(BaseModel):
    ...
    def from_money(cls, money: Money) -> "MoneyResponse":
        return cls(amount=str(money.amount), currency=money.currency.value)
...
async def search_investments_by_name(
    ...
    return InvestmentSearchResponse(
        investments=matching_investments, total=len(matching_investments)
    )
...
async def get_investment_by_symbol(
    symbol: str, current_user=Depends(get_current_active_user)
) -> InvestmentResponse:
...
async def delete_investment(
    investment_id: UUID, current_user=Depends(get_current_active_user)
) -> None:
...
async def search_investments(
    ...
    if symbol:
        all_investments = [
            inv for inv in all_investments if inv.symbol.upper() == symbol.upper()
        ]

    # Filter by sector if provided
    if sector:
        all_investments = [
            inv for inv in all_investments if inv.sector.lower() == sector.lower()
        ]
    ...
    if market_cap:
        all_investments = [
            inv
            for inv in all_investments
            if inv.market_cap.lower() == market_cap.lower()
        ]

    return InvestmentSearchResponse(
        investments=all_investments, total=len(all_investments)
    )

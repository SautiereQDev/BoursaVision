...
class YFinanceDataFetcher:
    ...
    async def fetch_historical_data(
        ...
        try:
            ...
            for index, row in hist.iterrows():
                ...
                point = TimelinePoint(
                    timestamp=timestamp,
                    open_price=Money(amount=Decimal(str(row['Open'])), currency=currency),
                    high_price=Money(amount=Decimal(str(row['High'])), currency=currency),
                    low_price=Money(amount=Decimal(str(row['Low'])), currency=currency),
                    close_price=Money(amount=Decimal(str(row['Close'])), currency=currency),
                    adjusted_close=Money(amount=Decimal(str(row.get('Adj Close', row['Close']))), currency=currency),
            ...
            logger.info(f"Fetched {len(points)} points for {symbol} ({period}, {interval})")
            return points
    ...
    def _generate_mock_data(self, symbol: str, period: str, interval: str) -> List[TimelinePoint]:

...
import numpy as np
import pandas as pd
from ..dtos import RiskAssessmentDTO, GeopoliticalRiskDTO, FundamentalRiskDTO
...
class MarketRiskAnalyzer(IRiskAnalyzer):
    ...
    async def analyze(self, symbol: str, market_data: Dict) -> List[RiskFactor]:
        ...
        try:
            ...
            if not spy_data.empty and len(hist) > 0:
                correlation = hist['Close'].pct_change().corr(spy_data['Close'].pct_change())
    ...
    def _assess_volatility_risk(self, volatility: float) -> RiskFactor:
        ...
        return RiskFactor(
            ...
            impact="HIGH" if volatility > 30 else "MEDIUM" if volatility > 15 else "LOW",
            probability="HIGH",
            timeframe="SHORT",
    ...
    def _assess_beta_risk(self, beta: float) -> RiskFactor:
        ...
        sensitivity = "très sensible" if beta > 1.5 else "sensible" if beta > 1.2 else "peu sensible"
...
class FundamentalRiskAnalyzer(IRiskAnalyzer):
    ...
    def _assess_revenue_quality_risk(self, info: Dict, financials: pd.DataFrame) -> Optional[RiskFactor]:
        ...
        try:
            ...
            total_revenue = financials.loc['Total Revenue'] if 'Total Revenue' in financials.index else None
...
class GeopoliticalRiskAnalyzer(IRiskAnalyzer):
    ...
    def _assess_country_risk(self, info: Dict) -> Optional[RiskFactor]:
        ...
        elif country in ['United States', 'Canada', 'Germany', 'Switzerland', 'Netherlands']:
            level = RiskLevel.VERY_LOW
            score = 15
        ...
        return RiskFactor(
            ...
            impact="HIGH" if level in [RiskLevel.HIGH, RiskLevel.VERY_HIGH] else "MEDIUM",
            probability="MEDIUM",
            timeframe="LONG",
    ...
    def _assess_sector_geopolitical_risk(self, info: Dict) -> Optional[RiskFactor]:
        ...
        high_risk_sectors = [
            'Energy', 'Oil & Gas', 'Defense', 'Technology', 'Semiconductors',
            'Telecommunications', 'Banking', 'Mining', 'Aerospace'
        ]
        
        # Industries spécifiquement sensibles
        high_risk_industries = [
            'Oil & Gas E&P', 'Semiconductors', 'Defense', 'Cybersecurity',
            'Telecommunications Equipment', 'Solar', 'Lithium', 'Rare Earth'
        ]
        
        is_high_risk = any(risk_sector.lower() in sector.lower() for risk_sector in high_risk_sectors)
        is_high_risk_industry = any(risk_industry.lower() in industry.lower() for risk_industry in high_risk_industries)
    ...
    def _assess_international_exposure_risk(self, info: Dict) -> Optional[RiskFactor]:
        ...
        if market_cap > 100_000_000_000 and is_tech:  # > 100B et tech
            level = RiskLevel.MODERATE
            score = 60
            description = "Forte exposition internationale - Sensible aux tensions commerciales"
...
class ESGRiskAnalyzer(IRiskAnalyzer):
    ...
    def _assess_environmental_risk(self, info: Dict) -> Optional[RiskFactor]:
        ...
        high_env_risk = [
            'Energy', 'Materials', 'Oil & Gas', 'Mining', 'Chemicals',
            'Steel', 'Aluminum', 'Coal', 'Utilities'
        ]
        
        is_high_risk = any(risk_sector.lower() in sector.lower() or 
                          risk_sector.lower() in industry.lower() 
    ...
    def _assess_social_risk(self, info: Dict) -> Optional[RiskFactor]:
        ...
        social_sensitive_sectors = [
            'Consumer Discretionary', 'Technology', 'Healthcare',
            'Financial Services', 'Communication Services'
        ]
        
        is_sensitive = any(sens_sector.lower() in sector.lower() 
                          for sens_sector in social_sensitive_sectors)
        
        if is_sensitive and employees > 100000:
            level = RiskLevel.MODERATE
            score = 55
            description = f"Grande entreprise dans secteur sensible ({employees:,} employés)"
...
class RiskAssessmentService:
    ...
    async def assess_comprehensive_risk(
        ...
        critical_risks = [risk for risk in all_risks 
    ...
    def _group_risks_by_category(self, risks: List[RiskFactor]) -> Dict[str, List[RiskFactor]]:
        """Grouper les risques par catégorie"""
        grouped = {}
        for risk in risks:
    ...
    def _generate_risk_summary(self, risks: List[RiskFactor], overall_level: RiskLevel) -> str:
        """Générer un résumé des risques"""
        if not risks:
            return "Aucune donnée de risque disponible"
        
        high_risks = [r for r in risks if r.level in [RiskLevel.HIGH, RiskLevel.VERY_HIGH]]

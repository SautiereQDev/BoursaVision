...
from .advanced_analysis_service import AdvancedInvestmentAnalyzer, ComprehensiveAnalysisResult
...
class InvestmentRecommendationService:
    ...
    def get_investment_recommendations(self, request: RecommendationRequest) -> PortfolioRecommendation:
        ...
        try:
            ...
            analysis_summary = self._generate_analysis_summary(analyses, filtered_analyses)
    ...
    def _analyze_investments_parallel(self, symbols: List[str]) -> List[ComprehensiveAnalysisResult]:
    ...
    def _filter_analyses(self, analyses: List[ComprehensiveAnalysisResult], request: RecommendationRequest) -> List[ComprehensiveAnalysisResult]:
        """Filter analyses based on request criteria."""
        filtered = []
    ...
    def _create_recommendation(self, analysis: ComprehensiveAnalysisResult) -> InvestmentRecommendation:
        ...
        if analysis.target_price and hasattr(analysis, 'current_price'):
            ...
            if current_price:
                upside_potential = ((analysis.target_price - current_price) / current_price) * 100
        ...
        key_insights = analysis.insights[:3] if len(analysis.insights) > 3 else analysis.insights
    ...
    def _calculate_portfolio_metrics(self, recommendations: List[InvestmentRecommendation]) -> Dict[str, Any]:
        ...
        return {
            ...
            "hold_count": len([r for r in recommendations if r.recommendation in ["HOLD", "WEAK_HOLD"]]),
            "high_quality_count": len([r for r in recommendations if r.data_quality in ["EXCELLENT", "GOOD"]])
        }
    
    def _generate_analysis_summary(self, all_analyses: List[ComprehensiveAnalysisResult], filtered_analyses: List[ComprehensiveAnalysisResult]) -> Dict[str, Any]:
        """Generate summary of analysis process."""
        return {
            "total_analyzed": len(all_analyses),
            "passed_filters": len(filtered_analyses),
            "filter_success_rate": len(filtered_analyses) / len(all_analyses) if all_analyses else 0,
            "average_score_all": sum(a.overall_score for a in all_analyses) / len(all_analyses) if all_analyses else 0,
            "average_score_filtered": sum(a.overall_score for a in filtered_analyses) / len(filtered_analyses) if filtered_analyses else 0,
            "analysis_timestamp": datetime.now(timezone.utc).isoformat()
        }
    
    def _assess_portfolio_risk(self, recommendations: List[InvestmentRecommendation]) -> Dict[str, Any]:
        ...
        avg_risk_score = sum(risk_scores.get(level, 50) for level in risk_levels) / len(risk_levels)
        
        return {
            "risk_distribution": risk_counts,
            "average_risk_score": avg_risk_score,
            "risk_assessment": self._categorize_portfolio_risk(avg_risk_score),
            "diversification_score": len(set(risk_levels)) / 5 * 100  # Max 5 risk levels
        }
    ...
    def _calculate_sector_allocation(self, recommendations: List[InvestmentRecommendation]) -> Dict[str, float]:
        ...
        for rec in recommendations:
            ...
            if any(tech in rec.symbol for tech in ['AAPL', 'MSFT', 'GOOGL', 'NVDA', 'META']):
                sector = "Technology"

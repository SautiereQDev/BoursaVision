...
class FullMarketStrategy(MarketScannerStrategy):
    ...
    SECTOR_SYMBOLS = {
        'technology': [
            ...
            'CRM', 'ORCL', 'INTC', 'AMD', 'CSCO', 'AVGO', 'TXN', 'QCOM', 'INTU', 'MU',
            'AMAT', 'LRCX', 'KLAC', 'MRVL', 'FTNT', 'TEAM', 'SNOW', 'PLTR', 'CRWD', 'ZS'
        ],
        'healthcare': [
            'JNJ', 'PFE', 'UNH', 'ABBV', 'TMO', 'ABT', 'LLY', 'DHR', 'BMY', 'MRK',
            'AMGN', 'GILD', 'ISRG', 'VRTX', 'REGN', 'BIIB', 'ILMN', 'MRNA', 'ZTS', 'CVS'
        ],
        'financial': [
            'JPM', 'BAC', 'WFC', 'GS', 'MS', 'C', 'AXP', 'USB', 'TFC', 'PNC',
            'COF', 'SCHW', 'BLK', 'SPGI', 'CME', 'ICE', 'MCO', 'AON', 'MMC', 'AJG'
        ],
        'consumer_discretionary': [
            'AMZN', 'TSLA', 'HD', 'MCD', 'NKE', 'LOW', 'SBUX', 'TJX', 'BKNG', 'CMG',
            'LULU', 'RCL', 'MAR', 'GM', 'F', 'NCLH', 'CCL', 'DIS', 'ABNB', 'ETSY'
        ],
        'consumer_staples': [
            'PG', 'KO', 'PEP', 'WMT', 'COST', 'MDLZ', 'CL', 'KMB', 'GIS', 'K',
            'HSY', 'MKC', 'SJM', 'CAG', 'CPB', 'CHD', 'CLX', 'TSN', 'HRL', 'KR'
        ],
        'energy': [
            'XOM', 'CVX', 'COP', 'EOG', 'SLB', 'PXD', 'KMI', 'OKE', 'WMB', 'MPC',
            'VLO', 'PSX', 'HES', 'FANG', 'DVN', 'BKR', 'HAL', 'CTRA', 'OXY', 'APA'
        ],
        'industrials': [
            'BA', 'CAT', 'HON', 'UPS', 'RTX', 'LMT', 'GE', 'MMM', 'DE', 'UNP',
            'FDX', 'NOC', 'GD', 'ETN', 'EMR', 'ITW', 'PH', 'CMI', 'ROK', 'DOV'
        ]
    }
...
class SectorStrategy(MarketScannerStrategy):
    ...
    async def get_symbols_to_scan(self, config: ScanConfig) -> List[str]:
        ...
        symbols = list(set(symbols) - config.exclude_symbols)
        logger.info(f"Scanning {len(symbols)} symbols for sectors: {self.target_sectors}")
        return symbols[:config.max_symbols]
...
class TechnicalAnalyzer:
    ...
    @staticmethod
    def calculate_technical_score(rsi: float, macd_signal: str, price: float, 
...
class FundamentalAnalyzer:
    ...
    def calculate_fundamental_score(
        ...
        metrics_count = sum(1 for x in [pe_ratio, pb_ratio, roe, debt_to_equity, dividend_yield] if x is not None)
...
class MarketScannerService:
    ...
    async def scan_market(self, config: ScanConfig) -> List[ScanResult]:
        ...
        logger.info(f"Market scan completed: {len(results)} symbols scanned in {scan_time:.2f}s")
        
        return results
    
    def _scan_single_symbol(self, symbol: str, config: ScanConfig, 
                                strategy: MarketScannerStrategy) -> Optional[ScanResult]:
    ...
    def _extract_scan_data(self, symbol: str, info: Dict[str, Any], 
                          hist: Any, config: ScanConfig) -> Optional[ScanResult]:
        ...
        try:
            ...
            if (market_cap and market_cap < config.min_market_cap) or volume < config.min_volume:
                return None
            
            # Calculer les métriques et scores
            fundamental_data = self._extract_fundamental_data(info)
            technical_data = self._extract_technical_data(hist) if config.include_technicals else {}
    ...
    def _extract_technical_data(self, hist: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            sma_200 = sum(prices[-200:]) / min(200, len(prices)) if prices else current_price
    ...
    def get_top_opportunities(self, results: List[ScanResult], limit: int = 50) -> List[ScanResult]:
        ...
        opportunities.sort(key=lambda x: (x.overall_score, x.fundamental_score), reverse=True)
        
        return opportunities[:limit]
    
    def get_sector_leaders(self, results: List[ScanResult]) -> Dict[str, List[ScanResult]]:

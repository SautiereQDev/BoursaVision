...
from .market_scanner import MarketScannerService, ScanConfig, ScanStrategy, ScanResult
...
class RecommendationStrategy(ABC):
    ...
    @abstractmethod
    def analyze_opportunity(self, scan_result: ScanResult) -> Optional[InvestmentRecommendation]:
...
class ValueInvestingStrategy(RecommendationStrategy):
    ...
    def analyze_opportunity(self, scan_result: ScanResult) -> Optional[InvestmentRecommendation]:
        ...
        potential_return = ((target_price - scan_result.price) / scan_result.price) * 100 if target_price else 0
...
class InvestmentIntelligenceService:
    ...
    async def generate_investment_recommendations(
        ...
        for strategy in self.strategies:
            ...
            for scan_result in scan_results:
                try:
                    ...
                    if recommendation and recommendation.confidence_score >= config.min_confidence_score:
                        strategy_recommendations.append(recommendation)
                except Exception as e:
                    logger.error(f"Error analyzing {scan_result.symbol} with {strategy.get_strategy_name()}: {e}")
            
            logger.info(f"{strategy.get_strategy_name()}: {len(strategy_recommendations)} recommendations generated")
        ...
        logger.info(f"Investment analysis completed: {len(recommendations)} final recommendations in {analysis_time:.2f}s")
    ...
    def _deduplicate_recommendations(
        ...
        for rec in recommendations:
            if rec.symbol not in best_recommendations:
                ...
                if rec.confidence_score > best_recommendations[rec.symbol].confidence_score:
                    best_recommendations[rec.symbol] = rec
    ...
    def _filter_and_rank_recommendations(
        ...
        for rec in recommendations:
            ...
            risk_levels = [RiskLevel.VERY_LOW, RiskLevel.LOW, RiskLevel.MODERATE, RiskLevel.HIGH, RiskLevel.VERY_HIGH]
            if risk_levels.index(rec.risk_level) > risk_levels.index(config.max_risk_level):
                continue
        ...
        filtered.sort(key=lambda x: (x.confidence_score, x.potential_return), reverse=True)
        
        return filtered
    
    def get_top_opportunities(self, limit: int = 20) -> List[InvestmentRecommendation]:
        """Récupère les meilleures opportunités"""
        strong_buys = [r for r in self.cached_recommendations if r.recommendation == RecommendationType.STRONG_BUY]
        buys = [r for r in self.cached_recommendations if r.recommendation == RecommendationType.BUY]
    ...
    def get_sector_recommendations(self) -> Dict[str, List[InvestmentRecommendation]]:
        ...
        for rec in self.cached_recommendations:
            if rec.sector and rec.recommendation in [RecommendationType.STRONG_BUY, RecommendationType.BUY]:
        ...
        for sector in sector_recommendations:
            sector_recommendations[sector].sort(key=lambda x: x.confidence_score, reverse=True)
            sector_recommendations[sector] = sector_recommendations[sector][:5]  # Top 5 par secteur
        
        return sector_recommendations
    
    def get_portfolio_suggestions(self, portfolio_size: float = 100000.0) -> Dict[str, Any]:
        ...
        return {
            ...
            "diversification_score": len(sector_groups) * 10,  # Plus de secteurs = meilleure diversification
            "expected_return": sum(s["potential_return"] * s["weight"] for s in portfolio_suggestions),
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        strong_buys = len([r for r in self.cached_recommendations if r.recommendation == RecommendationType.STRONG_BUY])
        buys = len([r for r in self.cached_recommendations if r.recommendation == RecommendationType.BUY])
        holds = len([r for r in self.cached_recommendations if r.recommendation == RecommendationType.HOLD])
        
        avg_confidence = sum(r.confidence_score for r in self.cached_recommendations) / len(self.cached_recommendations)
        avg_potential_return = sum(r.potential_return for r in self.cached_recommendations) / len(self.cached_recommendations)
        
        sectors_analyzed = len({r.sector for r in self.cached_recommendations if r.sector})
    ...
    def _get_top_sector(self) -> Optional[str]:
        ...
        buy_recommendations = [r for r in self.cached_recommendations 
                             if r.recommendation in [RecommendationType.STRONG_BUY, RecommendationType.BUY]]
        ...
        return max(sector_counts.items(), key=lambda x: x[1])[0] if sector_counts else None

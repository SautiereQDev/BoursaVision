...
if ADVANCED_ANALYSIS_AVAILABLE:
    ...
    def get_best_investment_recommendations(
        symbols: Optional[str] = Query(None, description="Comma-separated symbols to analyze (if None, analyzes all indices)"),
        ...
        exclude_sectors: Optional[str] = Query(None, description="Comma-separated sectors to exclude"),
        min_score: float = Query(60.0, ge=0, le=100, description="Minimum investment score"),
        max_risk_level: str = Query("HIGH", description="Maximum risk level: VERY_LOW, LOW, MODERATE, HIGH, VERY_HIGH")
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if exclude_sectors:
                exclude_sectors_list = [s.strip() for s in exclude_sectors.split(",") if s.strip()]

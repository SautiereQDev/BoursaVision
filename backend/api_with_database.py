...
class PortfolioDB(Base):
    ...
    investments = relationship(
        "InvestmentDB", back_populates="portfolio", cascade="all, delete-orphan"
    )
...
class PortfolioService:
    ...
    async def calculate_portfolio_value(self, portfolio_id: str, db: Session) -> float:
        ...
        investments = (
            db.query(InvestmentDB)
            .filter(InvestmentDB.portfolio_id == portfolio_id)
            .all()
        )
...
async def get_portfolios(db: Session = Depends(get_db)):
    ...
    for portfolio in portfolios:
        portfolio.total_value = await portfolio_service.calculate_portfolio_value(
            portfolio.id, db
        )
...
async def update_portfolio(
    portfolio_id: str, portfolio_data: PortfolioUpdate, db: Session = Depends(get_db)
):
...
async def create_investment(
    investment_data: InvestmentCreate, db: Session = Depends(get_db)
):
    """Créer un nouveau investissement"""
    # Vérifier que le portefeuille existe
    portfolio = (
        db.query(PortfolioDB)
        .filter(PortfolioDB.id == investment_data.portfolio_id)
        .first()
    )
    if not portfolio:
        raise HTTPException(status_code=404, detail=PORTFOLIO_NOT_FOUND)
...
async def update_investment(
    investment_id: str, investment_data: InvestmentUpdate, db: Session = Depends(get_db)
):
    ...
    # Mettre à jour la valeur du portefeuille
    portfolio = (
        db.query(PortfolioDB).filter(PortfolioDB.id == investment.portfolio_id).first()
    )
    if portfolio:
        portfolio.total_value = await portfolio_service.calculate_portfolio_value(
            portfolio.id, db
...
async def get_multiple_stock_prices(
    symbols: str = Query(
        description="Symboles séparés par des virgules (ex: AAPL,GOOGL,MSFT)"
    ),
):
...
def _generate_market_overview(market_data_cache: Dict[str, Any]) -> str:
    ...
    try:
        ...
        positive_ratio = (
            (positive_changes / total_stocks) * 100 if total_stocks > 0 else 0
        )
...
def _generate_analysis_summary(recommendations: List[InvestmentRecommendation]) -> str:
    ...
    try:
        ...
        buy_count = len(
            [r for r in recommendations if r.recommendation in ["STRONG_BUY", "BUY"]]
        )
...
async def get_investment_recommendations(
    risk_tolerance: str = Query(
        "MEDIUM", description="Tolérance au risque: LOW, MEDIUM, HIGH"
    ),
    investment_horizon: str = Query(
        "MEDIUM", description="Horizon d'investissement: SHORT, MEDIUM, LONG"
    ),
    sectors: Optional[str] = Query(
        None, description="Secteurs préférés séparés par des virgules"
    ),
    min_confidence: float = Query(
        60.0, description="Score de confiance minimum (0-100)"
    ),
):
    """
    ...
    for symbol in candidate_symbols:
        try:
            ...
            fundamental_analysis = await _generate_fundamental_analysis(
                symbol, stock_info
            )
            ...
            potential_return = (
                ((target_price - stock_data.price) / stock_data.price * 100)
                if target_price
                else 0
            )
            ...
            risk_level = _assess_risk_level(
                stock_info.sector, market_cap, fundamental_analysis
            )
...
async def assess_investment_risk(symbol: str):
    ...
    try:
        ...
        if not stock_data or not stock_info:
            raise HTTPException(
                status_code=404, detail=f"Données non disponibles pour {symbol}"
            )
        ...
        critical_risks = [
            r for r in risk_factors if r.level in ["HIGH", "VERY_HIGH", "CRITICAL"]
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur d'analyse des risques: {str(e)}"
        )
...
def _generate_mitigation_strategies(
    risk_factors: List[RiskFactor], overall_level: str
) -> List[str]:
    ...
    if overall_level in ["HIGH", "VERY_HIGH"]:
        strategies.append(
            "Considérer une position réduite ou un étalement dans le temps"
        )
...
def _generate_monitoring_recommendations(risk_factors: List[RiskFactor]) -> List[str]:
    ...
    if short_term_risks:
        recommendations.append(
            "Surveillance quotidienne des indicateurs de court terme"
        )

    medium_term_risks = [r for r in risk_factors if r.timeframe == "MEDIUM"]
    if medium_term_risks:
        recommendations.append(
            "Revue hebdomadaire des facteurs de risque à moyen terme"
        )
...
def _generate_risk_summary(risk_factors: List[RiskFactor], overall_level: str) -> str:
    ...
    summary += (
        f"Analyse de {len(risk_factors)} facteurs dans {len(categories)} catégories. "
    )
...
async def get_comprehensive_investment_analysis(symbol: str):
    ...
    try:
        ...
        risk_penalty = _calculate_risk_penalty(
            risk_assessment_response.overall_risk_score
        )
        ...
        final_recommendation = _determine_final_recommendation(
            overall_investment_score, risk_assessment_response.overall_risk_level
        )

        # 5. Évaluer le niveau de confiance
        confidence_level = _assess_confidence_level(
            overall_investment_score, risk_assessment_response.critical_risk_count
        )

        # 6. Générer les opportunités et risques clés
        key_opportunities = _extract_key_opportunities(
            technical_analysis, fundamental_analysis
        )
        ...
        return {
            ...
            "investment_strategies": _generate_investment_strategies(
                final_recommendation, overall_investment_score
            ),
            "position_sizing_guidance": _generate_position_sizing_guidance(
                risk_assessment_response.overall_risk_level
            ),
            "monitoring_schedule": _generate_monitoring_schedule(
                risk_assessment_response.all_risk_factors
            ),
            # Métadonnées
            "analysis_timestamp": datetime.now(),
            "data_sources": [
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur d'analyse complète: {str(e)}"
        )
...
def _extract_key_opportunities(technical: Dict, fundamental: Dict) -> List[str]:
    ...
    if technical["rsi"] <= 35:
        opportunities.append(
            f"Action en survente (RSI: {technical['rsi']:.1f}) - Opportunité d'achat"
        )
    ...
    if fundamental["pe_ratio"] < 15:
        opportunities.append(
            f"Valorisation attractive (P/E: {fundamental['pe_ratio']:.1f})"
        )
    ...
    if fundamental["revenue_growth"] > 15:
        opportunities.append(
            f"Forte croissance des revenus ({fundamental['revenue_growth']:.1f}%)"
        )
...
def _generate_investment_strategies(recommendation: str, score: float) -> List[str]:
    ...
    if recommendation in ["STRONG_BUY", "BUY"]:
        strategies.append(
            "Considérer un achat progressif par paliers (DCA - Dollar Cost Averaging)"
        )
        strategies.append(
            "Établir des ordres d'achat à des niveaux de support technique"
        )
        if score > 80:
            strategies.append(
                "Position de taille normale dans un portefeuille diversifié"
            )
        else:
        ...
        strategies.append(
            "Surveiller les niveaux de rebond technique pour optimiser la sortie"
        )
...
def _generate_position_sizing_guidance(risk_level: str) -> str:
    ...
    if risk_level in ["VERY_LOW", "LOW"]:
        ...
        return (
            "Position réduite (1-3% du portefeuille) - Surveillance active recommandée"
        )
...
async def get_portfolio_performance(portfolio_id: str, db: Session = Depends(get_db)):
    ...
    investments = (
        db.query(InvestmentDB).filter(InvestmentDB.portfolio_id == portfolio_id).all()
    )
        ...
        investment_details.append(
            {
                ...
                "weight_percent": round(
                    (investment_value / total_current_value * 100)

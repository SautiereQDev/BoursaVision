...
from boursa_vision.domain.entities.investment import InvestmentSector, InvestmentType, MarketCap
...
class TestInvestmentManagementScenario:
    ...
    async def test_complete_investment_lifecycle(self, test_client: AsyncClient, authenticated_user):
        """
...
class TestPortfolioManagementScenario:
    ...
    async def test_complete_portfolio_scenario(self, test_client: AsyncClient, authenticated_user):
        """
        ...
        update_position_data = {
            "quantity": 15  # Augmenter la quantité
        }
...
class TestInvestmentSearchScenario:
    ...
    async def test_comprehensive_investment_search(self, test_client: AsyncClient, authenticated_user):
        """
        ...
        investments_data = [
            {
                ...
                "investment_type": "STOCK", "sector": "TECHNOLOGY", "market_cap": "LARGE",
                "current_price": {"amount": "150.00", "currency": "USD"}
            },
            {
                "symbol": "JNJ", "name": "Johnson & Johnson",
                "investment_type": "STOCK", "sector": "HEALTHCARE", "market_cap": "LARGE",
                "current_price": {"amount": "160.00", "currency": "USD"}
            },
            {
                "symbol": "XOM", "name": "Exxon Mobil Corporation",
                "investment_type": "STOCK", "sector": "ENERGY", "market_cap": "LARGE",
                "current_price": {"amount": "90.00", "currency": "USD"}
            },
            {
                "symbol": "SMALLTECH", "name": "Small Tech Company",
                "investment_type": "STOCK", "sector": "TECHNOLOGY", "market_cap": "SMALL",
        ...
        johnson_found = any("Johnson" in inv["name"] for inv in search_results["investments"])
...
class TestUserJourneyScenario:
    ...
    async def test_complete_user_journey(self, test_client: AsyncClient):
        ...
        for inv_data in favorite_investments:
            response = await test_client.post("/api/v1/investments", json=inv_data, headers=headers)
            assert response.status_code == 201
            created_investments.append(response.json())
        ...
        portfolio_response = await test_client.post("/api/v1/portfolios", json=portfolio_data, headers=headers)
        assert portfolio_response.status_code == 201
        ...
        perf_response = await test_client.get(f"/api/v1/portfolios/{portfolio['id']}/value", headers=headers)
        ...
        price_updates = [
            {"investment_id": created_investments[0]["id"], "new_price": {"amount": "850.00", "currency": "USD"}},
            {"investment_id": created_investments[1]["id"], "new_price": {"amount": "130.00", "currency": "USD"}}
        ]
        ...
        updated_perf_response = await test_client.get(f"/api/v1/portfolios/{portfolio['id']}/value", headers=headers)
        ...
        delete_response = await test_client.delete(f"/api/v1/portfolios/{portfolio['id']}", headers=headers)
        assert delete_response.status_code == 204
        
        # Vérifier que le portefeuille n'existe plus
        get_deleted_response = await test_client.get(f"/api/v1/portfolios/{portfolio['id']}", headers=headers)

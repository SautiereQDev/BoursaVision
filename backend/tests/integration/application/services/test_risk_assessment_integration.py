...
class TestRiskAssessmentServiceIntegration:
    ...
    async def test_comprehensive_risk_assessment_with_real_classes(
    ...
    async def test_market_risk_analyzer_real_implementation(self, mock_ticker):
        ...
        volatile_prices = 100 + np.cumsum(
            rng_vol.standard_normal(100) * 0.05
        )  # Haute volatilité

        mock_hist_data = pd.DataFrame({"Close": volatile_prices}, index=dates)
        mock_ticker_instance.history.return_value = mock_hist_data
        mock_ticker.return_value = mock_ticker_instance
        ...
        if volatility_risk is not None:
            ...
            assert volatility_risk.level in [
                RiskLevel.VERY_LOW,
                RiskLevel.LOW,
                RiskLevel.MODERATE,
                RiskLevel.HIGH,
    ...
    async def test_fundamental_risk_analyzer_real_implementation(self, mock_ticker):
        ...
        assert isinstance(risks, list)
        debt_risk = next(
            (r for r in risks if "Dette" in r.name or "Debt" in r.name), None
        )
        # L'analyseur peut ne pas être entièrement implémenté
        if debt_risk is not None:
            # Accepter tout niveau de risque valide selon l'implémentation
            assert debt_risk.level in [
                RiskLevel.VERY_LOW,
                RiskLevel.LOW,
                RiskLevel.MODERATE,
                RiskLevel.HIGH,
    ...
    async def test_risk_categorization_and_scoring(self, mock_ticker):
        ...
        stable_prices = 100 + np.cumsum(
            rng_stable.standard_normal(252) * 0.01
        )  # Volatilité modérée

        mock_hist_data = pd.DataFrame({"Close": stable_prices}, index=dates)
        mock_ticker_instance.history.return_value = mock_hist_data
        mock_ticker_instance.financials = pd.DataFrame()
        mock_ticker_instance.balance_sheet = pd.DataFrame()
        mock_ticker.return_value = mock_ticker_instance
    ...
    @patch("yfinance.Ticker")
    @patch("yfinance.download")
    async def test_performance_with_real_implementation(
        self, mock_download, mock_ticker
    ):
        ...
        mock_download.return_value = pd.DataFrame({"Close": [100, 101, 102, 103, 104]})

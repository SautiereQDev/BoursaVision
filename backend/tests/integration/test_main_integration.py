...
class TestMainIntegration:
    ...
    def test_setup_paths_adds_correct_directories(self):
        ...
        try:
            ...
            assert any("src" in path for path in sys.path), "src directory should be in sys.path"
        finally:
            # Nettoyer
    ...
    def test_create_app_success(self):
        ...
        with patch('boursa_vision.main.setup_paths'):
            with patch.dict('sys.modules', {'fastapi_yfinance': MagicMock(app=mock_app)}):
                app = create_app()
    ...
    def test_main_handles_startup_errors(self, mock_exit):
        ...
        with patch('boursa_vision.main.detect_environment', side_effect=Exception("Config error")):
            main()

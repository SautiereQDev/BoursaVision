...
class TestInvestmentRepositoryQueries:
    ...
    async def test_should_find_investments_by_sector(self, test_session):
        ...
        healthcare_results = await repository.find_by_sector(InvestmentSector.HEALTHCARE)
    ...
    async def test_should_search_investments_by_name(self, test_session):
        ...
        inc_results = await repository.search_by_name("Inc")  # Doit trouver Apple et Amazon
...
class TestInvestmentRepositoryEdgeCases:
    ...
    async def test_should_handle_duplicate_symbol_gracefully(self, test_session):
        ...
        investment1 = InvestmentFactory.create(
            symbol="AAPL",
            name="Apple Inc."
        )
        
        investment2 = InvestmentFactory.create(
            symbol="AAPL",  # Même symbole
    ...
    async def test_should_maintain_data_integrity_across_transactions(self, test_session):
    ...
    async def test_should_handle_concurrent_operations(self, test_session):
        ...
        tasks = [save_investment(i) for i in range(5)]  # Réduire le nombre pour éviter les conflits
        ...
        concurrent_symbols = [inv.symbol for inv in all_investments if inv.symbol.startswith("CON")]

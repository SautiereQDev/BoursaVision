...
async def time_operation(
    operation_name: str, operation_func, monitor: PerformanceMonitor, *args, **kwargs
):
    ...
    try:
        ...
        latency_ms = (end_time - start_time) * 1000
        logger.warning(
            "Operation %s failed after %.2fms: %s", operation_name, latency_ms, e
        )
...
async def _run_user_queries(monitor: PerformanceMonitor, iterations: int = 100):
    ...
    for i in range(iterations // 10):  # Fewer iterations for bulk queries
        await time_operation("user_find_active", user_repo.find_active_users, monitor)
...
async def _run_concurrent_operations(
    monitor: PerformanceMonitor, concurrent_requests: int = 10
):
    ...
    logger.info(
        "Successful concurrent operations: %d/%d", successful, concurrent_requests
    )
...
def validate_acceptance_criteria(monitor: PerformanceMonitor) -> bool:
    ...
    for operation, _ in monitor.metrics.items():
        ...
        if p95 >= 100:
            logger.warning(
                "❌ %s: P95 %.2fms >= 100ms (fails acceptance criteria)", operation, p95
            )
            all_passed = False

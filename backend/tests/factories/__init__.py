...
class MoneyFactory(factory.Factory):
    ...
    amount = factory.LazyFunction(
        lambda: Decimal(str(fake.pyfloat(min_value=0.01, max_value=10000.0, right_digits=2)))
    )
...
class CurrencyFactory(factory.Factory):
    ...
    currency = factory.Iterator([
        ...
        Currency.CAD, Currency.JPY, Currency.CHF, Currency.AUD
    ])
...
class InvestmentFactory(factory.Factory):
    ...
    exchange = factory.Faker('random_element', elements=[
        'NYSE', 'NASDAQ', 'LSE', 'TSE', 'EURONEXT'
    ])
    currency = factory.Iterator([Currency.USD, Currency.EUR, Currency.GBP])
...
class PositionFactory(factory.Factory):
    ...
    def create_apple_position(cls, quantity: int = 100):
        ...
        return cls.create(
            symbol="AAPL",
            quantity=quantity,
            average_price=MoneyFactory.create(amount=Decimal("145.00"), currency=Currency.USD)
        )
...
class InvestmentModelFactory(factory.Factory):
    ...
    market_cap = factory.Faker('random_element', elements=[
        'NANO_CAP', 'MICRO_CAP', 'SMALL_CAP', 'MID_CAP', 'LARGE_CAP', 'MEGA_CAP'
    ])
...
class PortfolioModelFactory(factory.Factory):
    ...
    initial_cash = factory.LazyFunction(
        lambda: Decimal(str(fake.pyfloat(min_value=1000, max_value=100000, right_digits=2)))
    )
    ...
    total_value = factory.LazyFunction(
        lambda: Decimal(str(fake.pyfloat(min_value=1000, max_value=120000, right_digits=2)))
    )
    is_active = True
    created_at = factory.LazyFunction(
        lambda: fake.date_time_between(start_date='-1y', end_date='now', tzinfo=timezone.utc)
...
def create_test_portfolio_with_investments(user_id=None) -> tuple:
    ...
    for investment in investments:
        position = PositionFactory.create(
            ...
            average_price=Money(
                investment.current_price.amount * Decimal("0.95"),  # Bought at 5% discount
...
def create_realistic_market_scenario():
    ...
    tech_stocks = [
        InvestmentFactory.create(
            ...
            sector=InvestmentSector.TECHNOLOGY, market_cap=MarketCap.LARGE_CAP,
    ...
    healthcare_stocks = [
        InvestmentFactory.create(
            ...
            sector=InvestmentSector.HEALTHCARE, market_cap=MarketCap.LARGE_CAP,

...
class TestHealthCheckResponse:
    ...
    def test_health_check_response_with_failures(self):
        ...
        response = HealthCheckResponse(
            status="degraded",
            timestamp="2024-01-15T10:00:00Z",
            real_data_tests={"INVALID": {"success": False, "error": "No data found"}},
            summary={
...
class TestFastAPIEndpoints:
    ...
    def test_ticker_info_endpoint_no_data(self, mock_yf, client):
        ...
        assert response.status_code in [
            404,
            500,
        ]  # Accepter les deux car dépend du mock
    ...
    def test_ticker_history_endpoint_no_data(self, mock_yf, client):
        ...
        assert response.status_code == 500
        assert (
            "No historical data found" in response.json()["detail"]
            or "Error fetching history" in response.json()["detail"]
        )
...
class TestAdvancedAnalysisEndpoints:
    ...
    @pytest.mark.skipif(
        not ADVANCED_ANALYSIS_AVAILABLE, reason="Advanced analysis not available"
    )
    ...
    @pytest.mark.skipif(
        ADVANCED_ANALYSIS_AVAILABLE,
        reason="Test for when advanced analysis is not available",
    )
...
class TestAppConfiguration:
    ...
    def test_cors_middleware_present(self):
        ...
        cors_middleware_present = any(
            middleware.cls.__name__ == "CORSMiddleware" for middleware in middlewares
        )
...
class TestEdgeCases:
    ...
    def test_indices_endpoint_consistency(self, client):
        ...
        calculated_total = sum(
            len(index_data["symbols"]) for index_data in data["indices"].values()
        )

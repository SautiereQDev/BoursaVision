...
from boursa_vision.application.commands.investment.create_investment_command import CreateInvestmentCommand
from boursa_vision.application.commands.portfolio.create_portfolio_command import CreatePortfolioCommand
from boursa_vision.application.commands.portfolio.add_investment_to_portfolio_command import AddInvestmentToPortfolioCommand
from boursa_vision.application.commands.signal.generate_signal_command import GenerateSignalCommand
...
class TestCreateInvestmentCommandHandler:
    ...
    def test_handler_initialization(self, mock_investment_repository, mock_unit_of_work):
    ...
    async def test_handle_create_investment_success(
        self, handler, create_investment_command, mock_investment_repository, mock_unit_of_work
    ):
...
class TestCreatePortfolioCommandHandler:
    ...
    @pytest.fixture
    def handler(self, mock_portfolio_repository, mock_user_repository, mock_unit_of_work):
    ...
    async def test_handle_create_portfolio_success(
        ...
        mock_user_repository.find_by_id.assert_called_once_with(create_portfolio_command.user_id)
    ...
    async def test_handle_create_portfolio_minimal_data(
        self, mock_portfolio_repository, mock_user_repository, mock_unit_of_work, mock_user
    ):
...
class TestAddInvestmentToPortfolioCommandHandler:
    ...
    @pytest.fixture
    def handler(self, mock_portfolio_repository, mock_investment_repository, mock_unit_of_work):
    ...
    async def test_handle_add_investment_success(
        ...
        # Vérifications des appels
        mock_portfolio_repository.find_by_id.assert_called_once_with(add_investment_command.portfolio_id)
        mock_investment_repository.find_by_id.assert_called_once_with(add_investment_command.investment_id)
...
class TestGenerateSignalCommandHandler:
    ...
    def test_handler_initialization(self, mock_signal_generator):
        """Test l'initialisation du handler"""
        handler = GenerateSignalCommandHandler(
            signal_generator=mock_signal_generator
        )
    ...
    async def test_handle_signal_generation_error(
        ...
        # Arrange
        mock_signal_generator.generate_signal.side_effect = Exception("Signal generation failed")
...
class TestCommandHandlersIntegration:
    ...
    def test_handlers_dependency_injection(self):
        ...
        create_portfolio_handler = CreatePortfolioCommandHandler(mock_repo, mock_repo, mock_uow)
        add_investment_handler = AddInvestmentToPortfolioCommandHandler(mock_repo, mock_repo, mock_uow)

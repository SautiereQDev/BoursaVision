...
class TestArchiveDataProvider:
    ...
    def test_get_market_data_for_symbol_database_error(self):
        ...
        with patch(
            "psycopg2.connect", side_effect=Exception("Database connection failed")
        ):
            result = provider.get_market_data_for_symbol("AAPL")
    ...
    def test_get_available_symbols_success(self):
        ...
        mock_rows = [{"symbol": "AAPL"}, {"symbol": "GOOGL"}, {"symbol": "MSFT"}]
...
class TestArchiveEnhancedAdvancedAnalyzer:
    ...
    def enhanced_analyzer(self, mock_original_analyzer):
        ...
        with patch(
            "boursa_vision.application.services.archive_recommendation_service.ArchiveDataProvider"
        ):
    ...
    @pytest.mark.unit
    def test_analyze_investment_live_data_success(
        self, enhanced_analyzer, mock_original_analyzer
    ):
    ...
    @pytest.mark.unit
    def test_analyze_investment_live_data_error(
        self, enhanced_analyzer, mock_original_analyzer
    ):
        """Test analyze_investment with live data error, fallback to archive"""
        # Mock archive data available
        mock_archive_data = {"history": Mock(), "info": {"symbol": "AAPL"}}
        enhanced_analyzer.archive_provider.get_market_data_for_symbol.return_value = (
            mock_archive_data
        )
        ...
        enhanced_analyzer.archive_provider.get_market_data_for_symbol.assert_called_once_with(
            "AAPL"
        )
        assert result == mock_result

    @pytest.mark.unit
    def test_analyze_investment_no_archive_data(
        self, enhanced_analyzer, mock_original_analyzer
    ):
        """Test analyze_investment with no archive data available"""
        mock_original_analyzer.analyze_investment.side_effect = ConnectionError(
            "Live data failed"
        )
        enhanced_analyzer.archive_provider.get_market_data_for_symbol.return_value = (
            None
        ...
        with patch.dict(
            "sys.modules",
            {
                "boursa_vision.application.services.investment_recommendation_service": Mock(
                    InvestmentRecommendationService=MockServiceClass
                ),
                "boursa_vision.application.services.advanced_analysis_service": Mock(
                    AdvancedInvestmentAnalyzer=mock_analyzer_class
                ),
            },
        ):
...
class TestArchiveRecommendationServiceIntegration:
    ...
    def test_enhanced_analyzer_archive_fallback(self):
        ...
        mock_original_analyzer.analyze_investment.side_effect = Exception(
            "Live data failed"
        )
        ...
        with patch(
            ...
            assert result is None
            analyzer.archive_provider.get_market_data_for_symbol.assert_called_once_with(
                "AAPL"
            )

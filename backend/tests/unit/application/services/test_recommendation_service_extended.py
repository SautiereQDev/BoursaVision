...
class TestArchiveBasedRecommendationServiceInternal:
    ...
    def test_analyze_symbol_insufficient_data(self, service):
        ...
        service.repository.get_market_data.return_value = pd.DataFrame()  # Empty DataFrame
    ...
    def test_score_volume_increasing(self, service):
        ...
        # Arrange
        volume_analysis = {"trend": "INCREASING", "avg_volume": 1000000, "recent_vs_avg": 1.3}
    ...
    def test_score_volume_stable(self, service):
        ...
        # Arrange
        volume_analysis = {"trend": "STABLE", "avg_volume": 1000000, "recent_vs_avg": 1.0}
    ...
    def test_score_volume_decreasing(self, service):
        ...
        # Arrange
        volume_analysis = {"trend": "DECREASING", "avg_volume": 1000000, "recent_vs_avg": 0.7}
...
class TestRecommendationServiceMainFunction:
    ...
    def test_main_function_execution(self, mock_env_vars):
        ...
        # Arrange
        with patch('boursa_vision.application.services.recommendation.recommendation_service.ArchiveBasedRecommendationService') as mock_service_class:
            mock_service = Mock()
            mock_service.get_recommendations.return_value = [
                Mock(
                    symbol="TEST", recommendation="BUY", overall_score=85.0,
                    confidence=0.9, current_price=100.0, risk_level="LOW",
            ...
            from boursa_vision.application.services.recommendation.recommendation_service import main
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_analyze_volume_zero_volume(self, service):
        ...
        assert result['avg_volume'] == 0
        assert result['trend'] in ['INCREASING', 'DECREASING', 'STABLE']
        assert abs(result['recent_vs_avg'] - 1.0) < 0.01  # Should handle division by zero
    ...
    def test_full_analysis_workflow(self):
        ...
        if pd:
            ...
            test_data = pd.DataFrame({
                ...
                'volume': [1000000, 1100000, 1050000, 1200000, 1150000, 1300000, 1250000, 1400000]
            })
            service.repository.get_market_data.return_value = test_data
        else:
        ...
        # Act
        recommendations = service.get_recommendations(max_recommendations=1, min_score=0.0)

...
class TestMarketRiskAnalyzerCoverage:
    ...
    async def test_analyze_with_missing_beta(self, mock_download, mock_ticker, market_analyzer):
    ...
    @patch('yfinance.Ticker')
    @patch('yfinance.download')
    async def test_analyze_with_empty_spy_data(self, mock_download, mock_ticker, market_analyzer):
...
class TestFundamentalRiskAnalyzerCoverage:
    ...
    def test_assess_revenue_quality_risk_none_return(self, fundamental_analyzer):
        ...
        financials = pd.DataFrame({"Revenue": [100, 110, 120]})  # Non-empty but None revenue
...
class TestRiskAssessmentServiceCoverage:
    ...
    @patch('boursa_vision.application.services.risk_assessment.MarketRiskAnalyzer.analyze')
    @patch('boursa_vision.application.services.risk_assessment.FundamentalRiskAnalyzer.analyze')  
    async def test_assess_risk_with_exception_handling(self, mock_fundamental, mock_market, service):
...
class TestAnalyzerErrorHandling:
    ...
    async def test_geopolitical_analyzer_error_branches(self, mock_ticker):
        ...
        from boursa_vision.application.services.risk_assessment import GeopoliticalRiskAnalyzer
    ...
    def test_geopolitical_analyzer_extreme_cases(self):
        ...
        assert risk.level == RiskLevel.VERY_LOW  # Large companies typically have lower risk

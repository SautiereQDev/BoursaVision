...
from boursa_vision.domain.services.cache_strategies import CacheConfig, MarketDataCacheManager
...
class TestYFinanceDataFetcher:
    ...
    @patch('boursa_vision.application.services.market_data_cache_service.YF_AVAILABLE', True)
    def test_fetch_data_successful(self, fetcher):
        ...
        with patch('yfinance.Ticker') as mock_ticker:
            ...
            # Act
            result = asyncio.run(fetcher.fetch_historical_data(symbol, period, interval))
    ...
    @patch('boursa_vision.application.services.market_data_cache_service.YF_AVAILABLE', False)
    def test_fetch_data_yfinance_unavailable(self, fetcher):
        ...
        with patch('boursa_vision.application.services.market_data_cache_service.YF_AVAILABLE', False):
            # Act
            result = asyncio.run(fetcher.fetch_historical_data(symbol, period, interval))
    ...
    @pytest.mark.skip(reason="_convert_dataframe_to_timeline_points method not available in YFinanceDataFetcher API")
    ...
    @pytest.mark.skip(reason="_convert_dataframe_to_timeline_points method not implemented")
...
class TestMarketDataCacheServiceInitialization:
    ...
    def test_init_with_custom_config(self):
        ...
        service = MarketDataCacheService(
            ...
            timeline_repository=mock_repository
        )
    ...
    def test_stats_tracking_initialization(self):
        ...
        # Assert
        expected_stats = [
            "cache_hits", "cache_misses", "db_reads", 
            "db_writes", "yfinance_requests", "timelines_loaded"
        ]
...
class TestMarketDataCacheServiceTimelineManagement:
    ...
    async def test_get_timeline_memory_cache_hit(self, service):
        ...
        # Arrange
        symbol = "AAPL"
        mock_timeline = MarketDataTimeline(symbol=symbol, currency=Currency(code="USD", name="US Dollar", symbol="$"))
    ...
    async def test_get_timeline_database_not_found(self, service):
        ...
        assert result.currency == Currency(code="USD", name="US Dollar", symbol="$")  # Default
        assert symbol in service._timelines
        assert service.stats["timelines_loaded"] == 1
    ...
    async def test_get_timeline_force_refresh(self, service):
        ...
        # Arrange
        symbol = "REFRESH_TEST"
        old_timeline = MarketDataTimeline(symbol=symbol, currency=Currency(code="EUR", name="Euro", symbol="€"))
        service._timelines[symbol] = old_timeline
        
        new_timeline = MarketDataTimeline(symbol=symbol, currency=Currency(code="USD", name="US Dollar", symbol="$"))
...
class TestMarketDataCacheServiceDataRetrieval:
    ...
    async def test_get_market_data_with_timeline_cache_hit(self, service):
        ...
        # Arrange
        symbol = "CACHE_HIT"
        timeline = MarketDataTimeline(symbol=symbol, currency=Currency(code="USD", name="US Dollar", symbol="$"))
        
        # Add some test data to timeline
        test_point = TimelinePoint(
            timestamp=datetime(2024, 1, 1, tzinfo=timezone.utc),
            open_price=Money(Decimal('100.0'), Currency(code="USD", name="US Dollar", symbol="$")),
            high_price=Money(Decimal('105.0'), Currency(code="USD", name="US Dollar", symbol="$")),
            low_price=Money(Decimal('99.0'), Currency(code="USD", name="US Dollar", symbol="$")),
            close_price=Money(Decimal('104.0'), Currency(code="USD", name="US Dollar", symbol="$")),
            adjusted_close=Money(Decimal('104.0'), Currency(code="USD", name="US Dollar", symbol="$")),
            volume=1000000,
    ...
    async def test_get_market_data_fetch_from_yfinance(self, service):
        ...
        mock_points = [
            TimelinePoint(
                timestamp=datetime(2024, 1, 1, tzinfo=timezone.utc),
                open_price=Money(Decimal('150.0'), Currency(code="USD", name="US Dollar", symbol="$")),
                high_price=Money(Decimal('155.0'), Currency(code="USD", name="US Dollar", symbol="$")),
                low_price=Money(Decimal('148.0'), Currency(code="USD", name="US Dollar", symbol="$")),
                close_price=Money(Decimal('152.0'), Currency(code="USD", name="US Dollar", symbol="$")),
                adjusted_close=Money(Decimal('152.0'), Currency(code="USD", name="US Dollar", symbol="$")),
                volume=2000000,
                interval_type=IntervalType.ONE_DAY,
                source=DataSource.YFINANCE,
                precision_level=PrecisionLevel.HIGH
        ...
        with patch.object(service.fetcher, 'fetch_and_convert', return_value=mock_points) as mock_fetch:
            # Act
            result = await service.get_market_data(
                symbol=symbol,
    ...
    async def test_get_market_data_default_time_range(self, service):
        ...
        with patch.object(service.fetcher, 'fetch_and_convert', return_value=[]) as mock_fetch:
            # Act
    ...
    async def test_get_market_data_different_intervals(self, service):
        ...
        with patch.object(service.fetcher, 'fetch_and_convert', return_value=[]) as mock_fetch:
            ...
            for interval in [IntervalType.ONE_MINUTE, IntervalType.ONE_HOUR, IntervalType.ONE_DAY]:
...
class TestMarketDataCacheServiceAdvancedFeatures:
    ...
    def test_get_cache_stats_comprehensive(self, service):
        ...
        with patch.object(service.cache_manager, 'get_stats', return_value={"manager_hits": 8}):
            # Act
            stats = service.get_cache_stats()
...
class TestMarketDataCacheServiceIntegration:
    ...
    @pytest.mark.skip(reason="Complex async mocking issue with fetch_historical_data - need to refactor service implementation")
    @pytest.mark.asyncio
    async def test_complete_data_retrieval_workflow(self, service_with_all_deps):
        ...
        with patch.object(service.fetcher, 'fetch_historical_data', new_callable=AsyncMock, return_value=mock_points):
            # Act
            result = await service.get_market_data(symbol)
    ...
    async def test_error_recovery_workflow(self, service_with_all_deps):
        ...
        with patch.object(service.fetcher, 'fetch_historical_data', new_callable=AsyncMock, return_value=[]):
    ...
    @pytest.mark.skip(reason="get_memory_usage method not implemented in MarketDataCacheService")

...
from boursa_vision.domain.value_objects.auth import TokenPair, AccessToken, RefreshToken as RefreshTokenVO
...
class TestUserAuthentication:
    ...
    async def test_should_authenticate_user_with_valid_credentials(
        ...
        mock_dependencies['user_repository'].find_by_email_for_auth.return_value = mock_user
        mock_dependencies['password_service'].verify_password.return_value = True
        mock_dependencies['jwt_service'].create_token_pair.return_value = mock_token_pair
        
        # Mock refresh token creation
        with patch('boursa_vision.domain.entities.refresh_token.RefreshToken.create') as mock_rt_create:
        ...
        mock_dependencies['user_repository'].find_by_email_for_auth.assert_called_once_with(email)
...
class TestTokenRefresh:
    ...
    async def test_should_refresh_token_with_valid_refresh_token(self):
        ...
        mock_new_token_pair.refresh_token.expires_at = datetime.now(timezone.utc) + timedelta(days=7)
        ...
        assert mock_token_entity.expires_at == mock_new_token_pair.refresh_token.expires_at
...
class TestUserLogout:
    ...
    async def test_should_handle_logout_when_user_not_found(self, auth_service_with_mocks):
    ...
    async def test_should_handle_logout_when_refresh_token_not_found(self, auth_service_with_mocks):
...
class TestAccessTokenValidation:
    ...
    async def test_should_return_none_for_token_without_subject(self, auth_service_with_mocks):
    ...
    async def test_should_return_none_for_invalid_user_id_format(self, auth_service_with_mocks):
    ...
    async def test_should_return_none_when_user_not_found(self, auth_service_with_mocks):
...
class TestUserRegistration:
    ...
    async def test_should_register_new_user_successfully(self, auth_service_with_mocks):
        ...
        with patch('boursa_vision.domain.entities.user.User.create', return_value=mock_user):
            # Act
    ...
    async def test_should_raise_error_when_username_exists(self, auth_service_with_mocks):
...
class TestPasswordChange:
    ...
    async def test_should_change_password_successfully(self, auth_service_with_mocks):
        ...
        user_repo.find_by_id.assert_called_once_with(user_id)
        password_service.verify_password.assert_called_once_with(old_password, "old_hash")
        password_service.hash_password.assert_called_once_with(new_password)
        assert mock_user.password_hash == "new_hash"
        user_repo.save.assert_called_once_with(mock_user)

    async def test_should_raise_error_when_user_not_found(self, auth_service_with_mocks):
    ...
    async def test_should_raise_error_when_old_password_incorrect(self, auth_service_with_mocks):

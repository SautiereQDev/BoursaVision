...
class TestAuthenticateUser:
    ...
    async def test_authenticate_user_success(
        ...
        with patch('boursa_vision.application.services.authentication_service.RefreshTokenEntity') as MockRefreshToken:
        ...
        mock_dependencies['user_repo'].find_by_email_for_auth.assert_called_once_with("test@example.com")
        mock_dependencies['password_service'].verify_password.assert_called_once_with("password123", "hashed_password")
        ...
        mock_dependencies['refresh_token_repo'].save.assert_called_once_with(mock_refresh_token)
...
class TestRefreshAccessToken:
    ...
    async def test_refresh_access_token_success(self, auth_service_with_mocks):
        ...
        mocks['refresh_token_repo'].find_by_token.assert_called_once_with("refresh_token")
...
class TestRegisterUser:
    ...
    async def test_register_user_success(self, auth_service_with_mocks):
        ...
        with patch('boursa_vision.application.services.authentication_service.User') as MockUser:
...
class TestChangePassword:
    ...
    async def test_change_password_success(self, auth_service_with_mocks):
        ...
        mocks['user_repo'].find_by_id.assert_called_once_with(user_id)
        mocks['password_service'].verify_password.assert_called_once_with("old_password", "old_hashed_password")
    ...
    @pytest.mark.asyncio
    async def test_change_password_incorrect_old_password(self, auth_service_with_mocks):
...
class TestCleanupExpiredTokens:
    ...
    async def test_cleanup_expired_tokens_success(self, auth_service_with_mocks):
        ...
        # Assertions
        assert result == 5
        mocks['refresh_token_repo'].cleanup_expired_tokens.assert_called_once_with(mock_now)
...
class TestAuthenticationServiceIntegration:
    ...
    async def test_complete_authentication_flow(self, full_auth_service_mock):
        ...
        mock_token_pair.refresh_token.expires_at = datetime.now(timezone.utc) + timedelta(days=7)

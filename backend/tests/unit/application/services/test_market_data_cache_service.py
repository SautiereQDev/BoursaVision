...
class TestMarketDataCacheServiceBasics:
    ...
    async def test_get_latest_price_exists(self):
        ...
        service = MarketDataCacheService()
        mock_timeline = Mock()
        mock_price = Money(Decimal("100.0"), Currency(code="USD", name="US Dollar", symbol="$"))
...
class TestYFinanceDataFetcher:
    ...
    def test_init(self):
        ...
        assert abs(fetcher.rate_limit_delay - 0.1) < 0.01  # Use approximate equality for floats
...
class TestMarketDataCacheServiceIntegration:
    ...
    async def test_get_market_data_basic_flow(self):
        ...
        with patch.object(service.fetcher, 'fetch_historical_data', return_value=mock_points):
    ...
    async def test_cleanup_old_data_with_repository(self):
        ...
        timeline = MarketDataTimeline(symbol="CLEANUP_TEST", currency=Currency(code="USD", name="US Dollar", symbol="$"))
    ...
    async def test_get_timeline_with_database_success(self):
        ...
        mock_repo = AsyncMock()
        mock_timeline = MarketDataTimeline(symbol="DB_TEST", currency=Currency(code="USD", name="US Dollar", symbol="$"))

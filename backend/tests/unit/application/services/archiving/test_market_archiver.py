...
class TestEnhancedMarketDataArchiver:
    ...
    def test_should_initialize_with_default_configuration(
        self, mock_database_components
    ):
        # Arrange & Act
    ...
    def test_should_fetch_and_process_symbol_data_successfully(
        self, archiver, mock_database_components
    ):
        # Arrange
        symbol = "TEST"
        interval = "1d"
    ...
    @patch(
        "sys.modules",
        {"boursa_vision.application.services.archiving.market_archiver.yf": Mock()},
    )
    def test_should_handle_database_integrity_error(
        ...
        with patch(
            ...
            mock_session.commit.side_effect = IntegrityError(
                statement="INSERT INTO ...", params={}, orig=Exception("duplicate key")
            )
            archiver.session_local = Mock(return_value=mock_session)
    ...
    @patch("time.sleep")
    def test_should_archive_all_symbols_with_delay(
        self, mock_sleep, mock_fetch_method, archiver
    ):
        # Arrange
    ...
    def test_should_get_archive_stats_from_database(
        self, archiver, mock_database_components
    ):
        # Arrange
        mock_session = Mock()
    ...
    def test_should_detect_potential_duplicates(
        self, archiver, mock_database_components
    ):
        # Arrange
        mock_session = Mock()
    ...
    def test_should_not_detect_duplicates_with_same_prices(
        self, archiver, mock_database_components
    ):
        # Arrange
        mock_session = Mock()

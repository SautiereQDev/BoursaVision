...
from boursa_vision.domain.entities.investment import Investment, InvestmentType, InvestmentSector, MarketCap
...
class TestAnalysisWeights:
    ...
    def test_analysis_weights_default_values(self):
        ...
        total = (weights.technical + weights.fundamental + weights.momentum + 
                weights.value + weights.growth + weights.quality)
...
class TestTechnicalAnalysisStrategy:
    ...
    def test_rsi_score_extreme_values(self, strategy):
        ...
        history_overbought = pd.DataFrame({
            'Date': dates,
            'Close': rising_prices
        })
...
class TestAdvancedInvestmentAnalyzer:
    ...
    def test_analyze_investment_complete_analysis(self, mock_ticker, analyzer, investment):
        ...
        assert result.recommendation in ['BUY', 'STRONG_BUY', 'HOLD', 'WEAK_HOLD', 'SELL', 'STRONG_SELL']
        assert result.risk_level in ['VERY_LOW', 'LOW', 'MODERATE', 'HIGH', 'VERY_HIGH']
        assert 0 <= result.confidence <= 1
    
    @patch('yfinance.Ticker')
    def test_analyze_investment_with_custom_weights(self, mock_ticker, analyzer, investment):
    ...
    @patch('yfinance.Ticker')
    def test_analyze_investment_with_missing_data(self, mock_ticker, analyzer, investment):
    ...
    def test_overall_score_calculation_logic(self, analyzer):
        ...
        total_weight = (weights.technical + weights.fundamental + weights.momentum + 
                       weights.value + weights.growth + weights.quality)
...
class TestComprehensiveIntegration:
    ...
    def test_multiple_investment_comparison(self, analyzer):
        ...
        assert all(0 <= score <= 100 for score in scores)  # Tous les scores sont dans la plage valide

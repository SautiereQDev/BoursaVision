...
class TestSqlAlchemyMarketDataTimelineRepository:
    ...
    @pytest.mark.unit
    @pytest.mark.skipif(not HAS_SQLALCHEMY, reason="SQLAlchemy specific test - needs real IntegrityError")
    async def test_bulk_save_points_integrity_error(self):
        ...
        if HAS_SQLALCHEMY:
            try:
                ...
                integrity_error = IntegrityError("INSERT statement", {}, Exception("Duplicate key"))
            except ImportError:
...
class TestRepositoryEdgeCases:
    ...
    async def test_bulk_save_points_partial_success(self):
        ...
        if HAS_SQLALCHEMY:
            try:
                from sqlalchemy.exc import IntegrityError
                integrity_error = IntegrityError("INSERT statement", {}, Exception("Duplicate"))
            except ImportError:
                class IntegrityError(Exception):
                    pass

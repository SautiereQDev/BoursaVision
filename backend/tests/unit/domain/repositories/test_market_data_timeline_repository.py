...
class TestSqlAlchemyMarketDataTimelineRepository:
    ...
    async def test_get_points_in_range_no_interval(self):
        ...
        points = await self.repository.get_points_in_range("AAPL", start_time, end_time)
    ...
    @pytest.mark.skipif(
        not HAS_SQLALCHEMY,
        reason="SQLAlchemy specific test - needs real IntegrityError",
    )
    async def test_bulk_save_points_integrity_error(self):
        """Test sauvegarde avec erreur d'intégrité"""
        # Importer IntegrityError du repository ou créer une classe qui hérite
        if HAS_SQLALCHEMY:
            try:
                from sqlalchemy.exc import IntegrityError

                integrity_error = IntegrityError(
                    "INSERT statement", {}, Exception("Duplicate key")
                )
            except ImportError:
            ...
            # Fallback: créer une classe qui simule IntegrityError
            class IntegrityError(Exception):
                pass
        ...
        points = [create_sample_point()]
        self.mock_session.merge.side_effect = integrity_error
    ...
    async def test_get_symbols_with_data_since_filter(self):
        ...
        symbols = await self.repository.get_symbols_with_data(min_points=5, since=since)
...
class TestRepositoryEdgeCases:
    ...
    async def test_get_timeline_case_insensitive(self):
        ...
        assert timeline is None
        # Vérifier que la méthode execute a été appelée (le symbole est converti en majuscule dans la requête)
    ...
    async def test_bulk_save_points_partial_success(self):
        ...
        if HAS_SQLALCHEMY:
            try:
                ...
                integrity_error = IntegrityError(
                    "INSERT statement", {}, Exception("Duplicate")
                )
            except ImportError:
                ...
                integrity_error = IntegrityError("Duplicate")
        else:
            # Fallback: créer une classe qui simule IntegrityError
            class IntegrityError(Exception):
                pass
        ...
        self.mock_session.merge.side_effect = [Mock(), integrity_error]

...
# Imports à tester
from boursa_vision.domain.value_objects.price import Price, PricePoint, PriceData
...
class TestPrice:
    ...
    def test_price_change_from_different_currency_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Cannot compare prices in different currencies"):
...
class TestPricePoint:
    ...
    def test_price_point_invalid_ohlc_open_too_high(self):
        ...
        with pytest.raises(ValueError, match="Invalid OHLC: open price not between high and low"):
            PricePoint(
                timestamp=self.timestamp,
    ...
    def test_price_point_invalid_ohlc_close_too_high(self):
        ...
        with pytest.raises(ValueError, match="Invalid OHLC: close price not between high and low"):
            PricePoint(
                timestamp=self.timestamp,
                open_price=self.open_price,
                high_price=self.high_price,
...
class TestPriceData:
    ...
    def test_price_data_non_chronological_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Price points must be in chronological order"):
            PriceData(
                symbol=self.symbol,
    ...
    def test_price_data_mixed_currencies_raises_error(self):
        ...
        with pytest.raises(ValueError, match="All price points must be in the same currency"):
            PriceData(
                symbol=self.symbol,
                points=mixed_points,
                interval=self.interval
    ...
    def test_price_data_period_return(self):
        ...
        expected_return = price_data.latest_price.change_percentage_from(price_data.oldest_price)
    ...
    def test_price_data_get_returns(self):
        ...
        expected_first_return = (Decimal("103.00") - Decimal("102.00")) / Decimal("102.00")

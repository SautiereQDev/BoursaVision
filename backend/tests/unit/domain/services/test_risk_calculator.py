...
from boursa_vision.domain.entities.investment import Investment, InvestmentSector, MarketCap
...
def sample_portfolio():
    ...
    portfolio.calculate_total_value.return_value = Money(Decimal("100000.00"), Currency.USD)
    return portfolio
...
def sample_portfolio_risk_input(
    sample_portfolio, sample_positions, sample_investments,
    sample_current_prices, sample_historical_returns
):
...
class TestRiskCalculatorRiskValidation:
    ...
    def test_should_validate_risk_limits_successfully(
        self, risk_calculator, sample_portfolio, sample_positions,
        sample_investments, sample_current_prices, sample_risk_limits
    ):
        """Test la validation réussie des limites de risque."""
        # Act
        result = risk_calculator.validate_risk_limits(
            sample_portfolio, sample_positions, sample_investments,
            sample_current_prices, sample_risk_limits
        )
    ...
    def test_should_detect_position_limit_violation(
        self, risk_calculator, sample_portfolio, sample_investments,
        sample_current_prices, sample_risk_limits
    ):
        ...
        # Act
        result = risk_calculator.validate_risk_limits(
            sample_portfolio, positions, sample_investments,
            sample_current_prices, sample_risk_limits
        )
    ...
    def test_should_generate_warnings_for_near_limits(
        ...
        prices_with_financial = {**sample_current_prices, "JPM": Money(Decimal("150.00"), Currency.USD)}
        ...
        sample_risk_limits.max_sector_exposure = 40.0  # Tech 15% et Financial 15% < 40% ✓
        sample_risk_limits.min_cash_percentage = 5.0  # 10% > 5% ✓
        
        # Act
        result = risk_calculator.validate_risk_limits(
            sample_portfolio, positions, investments_with_financial,
            prices_with_financial, sample_risk_limits
        )
        
        # Assert
        assert result.is_valid is True  # Pas de violations avec des positions modérées et diversifiées
...
class TestRiskCalculatorRiskReduction:
    ...
    def test_should_suggest_reducing_oversized_positions(
        ...
        assert any("Reduce" in suggestion and "position" in suggestion 
                  for suggestion in suggestions)
    ...
    def test_should_suggest_reducing_high_risk_positions(
        ...
        # Act
        suggestions = risk_calculator.suggest_risk_reduction(
            sample_portfolio, positions, investments,
            current_prices, sample_risk_limits
        )
...
class TestRiskCalculatorPrivateMethods:
    ...
    def test_should_calculate_position_weight(self, risk_calculator):
        ...
        with patch.object(risk_calculator, '_calculate_position_weight') as mock_method:
            ...
            weight = risk_calculator._calculate_position_weight(position, portfolio_value)
...
class TestPortfolioRiskInputDataclass:
    ...
    def test_should_create_portfolio_risk_input(
        self, sample_portfolio, sample_positions, sample_investments,
        sample_current_prices, sample_historical_returns
    ):

...
class TestArchiveBasedRecommendationService:
    ...
    def test_get_recommendations_success_with_valid_data(
        ...
        results = recommendation_service.get_recommendations(
            max_recommendations=5, min_score=50.0
        )
    ...
    @pytest.mark.unit
    def test_get_recommendations_empty_symbols_list(
    ...
    def test_get_recommendations_filters_low_scores(
        ...
        mock_technical_analyzer.calculate_moving_averages.return_value = {
            ...
            "ma_10": 98.0,
            "ma_20": 100.0,
            "ma_50": 102.0,  # Bearish MA
        }
        mock_technical_analyzer.calculate_volatility.return_value = (
            25.0  # High volatility
        )
    ...
    def test_get_recommendations_handles_analysis_errors(
        ...
        mock_repository.get_symbol_info.return_value = {
            "symbol": "VALID_SYMBOL",
            "name": "Valid Corp",
        }
    ...
    def test_calculate_confidence(self, recommendation_service, sample_market_data):
        ...
        # Act
        confidence = recommendation_service._calculate_confidence(
            sample_market_data, rsi, volatility
        )
    ...
    def test_generate_insights(self, recommendation_service):
        ...
        rsi_strength_found = any(
            "oversold" in strength.lower() for strength in strengths
        )
        assert (
            rsi_strength_found or len(strengths) > 0
        )  # Allow for different insight generation

        # Should have insights about volume
        volume_insight_found = any(
            "volume" in insight.lower() for insight in strengths + insights
        )
    ...
    def test_edge_cases_with_minimal_data(self, recommendation_service):
        ...
        price_changes = recommendation_service._calculate_price_changes(
            minimal_data["close"]
        )

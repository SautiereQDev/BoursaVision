        ...
        def database_manager(self):
            class DummyDB:
                def get_session(self):
                    ...
                    return DummySession()
            return DummyDB()
    monkeypatch.setattr("src.infrastructure.persistence.config.PersistenceInitializer", DummyInit)
    async def dummy_aenter(self):
        return DummyInit()
    monkeypatch.setattr("src.infrastructure.persistence.config.PersistenceContext.__aenter__", dummy_aenter)
    monkeypatch.setattr("src.infrastructure.persistence.config.initialize_persistence", lambda config: DummyInit())
    monkeypatch.setattr("src.infrastructure.persistence.config.get_persistence_initializer", lambda: DummyInit())
    async def dummy_shutdown():
        pass
    monkeypatch.setattr("src.infrastructure.persistence.config.shutdown_persistence", dummy_shutdown)

...
from src.infrastructure.persistence.factory import (
    SqlAlchemyRepositoryFactory, RepositoryFactoryProvider, RepositoryRegistry
)
...
async def test_sqlalchemy_repository_factory_creates_repositories(monkeypatch):
    ...
    monkeypatch.setattr('src.infrastructure.persistence.factory.SQLAlchemyUserRepository', MagicMock)
    monkeypatch.setattr('src.infrastructure.persistence.factory.SQLAlchemyPortfolioRepository', MagicMock)
    monkeypatch.setattr('src.infrastructure.persistence.factory.SQLAlchemyMarketDataRepository', MagicMock)
    monkeypatch.setattr('src.infrastructure.persistence.factory.SQLAlchemyInvestmentRepository', MagicMock)

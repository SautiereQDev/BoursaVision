...
class BestInvestmentsFinder:
    ...
    def get_market_scan(self, max_symbols: int = 50) -> Dict:
        ...
        response = self.session.post(
            f"{self.api_url}/api/v1/market/scan", 
            json=scan_request,
            timeout=120
        )
    ...
    def get_investment_recommendations(self, min_confidence: float = 75.0) -> Dict:
        ...
        print(f"🤖 Génération des recommandations IA (confiance min: {min_confidence}%)...")
    ...
    def get_sector_analysis(self) -> Dict:
        ...
        response = self.session.get(
            f"{self.api_url}/api/v1/market/sectors",
            timeout=60
        )
...
╚══════════════════════════════════════════════════════════════════════════════╝
    ...
    def display_top_opportunities(self, scan_results: Dict, limit: int = 10):
        ...
        print(f"📈 {len(opportunities)} meilleures opportunités sur {scan_results['scan_summary']['total_analyzed']} actions analysées\n")
        
        # En-tête du tableau
        print(f"{'Rang':<4} {'Symbole':<8} {'Nom':<25} {'Prix':<10} {'Score':<8} {'P/E':<8} {'RSI':<8} {'Secteur':<15}")
        print("-" * 95)
        ...
        for i, opp in enumerate(opportunities, 1):
            ...
            print(f"{indicator}{i:<3} {opp['symbol']:<8} {opp['name'][:24]:<25} {prix:<10} {score:<8} {pe:<8} {rsi:<8} {opp['sector'][:14]:<15}")
    ...
    def display_buy_recommendations(self, recommendations: Dict, limit: int = 5):
        ...
        for i, rec in enumerate(buys, 1):
            print(f"{i}. ✅ {rec['symbol']} - {rec['name']}")
            print(f"   💰 ${rec['current_price']:.2f} → ${rec['target_price']:.2f} (+{rec['potential_return']:.1f}%)")
            print(f"   📊 Confiance: {rec['confidence_score']:.1f}% | Risque: {rec['risk_level']}")
            print()
    ...
    def display_portfolio_suggestion(self, portfolio: Dict):
        ...
        print(f"📈 Rendement attendu: {summary.get('expected_annual_return', 0):.1f}% par an")
        print(f"🎯 Score diversification: {summary.get('diversification_score', 0):.0f}/100")
        print(f"⚠️  Tolérance risque: {summary.get('risk_level', 'N/A')}")
        print()
        
        if allocations:
            print("📋 Allocation suggérée:")
            print(f"{'Symbole':<8} {'Nom':<20} {'Poids':<8} {'Montant':<12} {'Actions':<8} {'Secteur':<15}")
            print("-" * 75)
            ...
            for alloc in allocations:
                ...
                print(f"{alloc['symbol']:<8} {alloc['name'][:19]:<20} {poids:<8} {montant:<12} {actions:<8} {alloc['sector'][:14]:<15}")
    ...
    def display_sector_analysis(self, sectors: Dict, limit: int = 5):
        ...
        for ranking in rankings:
            ...
            print(f"   🎯 Meilleur choix: {ranking['top_pick']} (Score: {ranking['top_pick_score']:.1f})")
    ...
    def display_summary_and_advice(self, scan_results: Dict, recommendations: Dict):
        ...
        print(f"   • Opportunités trouvées: {scan_summary.get('top_opportunities_found', 0)}")
        print(f"   • Score moyen du marché: {scan_summary.get('average_score', 0):.1f}/100")
        print(f"   • Recommandations STRONG_BUY: {analysis_summary.get('strong_buy_count', 0)}")
        ...
        print()
        print("⚠️  AVERTISSEMENT: Ces analyses sont automatisées et à des fins éducatives.")
    ...
    def run_complete_analysis(self, portfolio_size: float = 100000.0):
        ...
        if not self.check_api_status():
            print("❌ ERREUR: L'API n'est pas accessible. Assurez-vous qu'elle est lancée.")
        ...
        try:
            ...
            print("🔧 Vérifiez que l'API est bien lancée avec: cd backend && ./launch_api.sh")

def main():
    """Fonction principale"""
    import argparse
    
    parser = argparse.ArgumentParser(description="Identifier les meilleurs placements avec Boursa Vision")
    parser.add_argument("--portfolio", type=float, default=100000.0, 
                       help="Taille du portefeuille en USD (défaut: 100000)")
    parser.add_argument("--api-url", default="http://localhost:8005",

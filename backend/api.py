...
class InvestmentRequest(BaseModel):
    min_confidence_score: float = Field(
        default=70.0, description="Score de confiance minimum"
    )
    max_recommendations: int = Field(
        default=20, description="Nombre max de recommandations"
    )
    min_market_cap: float = Field(default=1000000000, description="Cap minimum")
...
class PortfolioRequest(BaseModel):
    portfolio_size: float = Field(
        default=100000.0, description="Taille du portefeuille"
    )
...
class MarketScanner:
    ...
    def scan_market(self, request: MarketScanRequest) -> Dict:
        ...
        return {
            "scan_summary": {
                ...
                "top_opportunities_found": len(
                    [r for r in results if r["overall_score"] > 75]
                ),
    ...
    def _meets_criteria(self, data: Dict, request: MarketScanRequest) -> bool:
        ...
        return (
            data["market_cap"] >= request.min_market_cap
            and data["volume"] >= request.min_volume
            and data["pe_ratio"] > 0
        )
...
class InvestmentIntelligence:
    ...
    def generate_recommendations(self, request: InvestmentRequest) -> Dict:
        ...
        scan_request = MarketScanRequest(
            max_symbols=50, min_market_cap=request.min_market_cap
        )
        ...
        strong_buy = [
            ...
            if isinstance(r, dict) and r.get("recommendation") == "STRONG_BUY"
        ]
        buy = [
            r
            for r in recommendations
            if isinstance(r, dict) and r.get("recommendation") == "BUY"
        ]
        hold = [
            r
            for r in recommendations
            if isinstance(r, dict) and r.get("recommendation") == "HOLD"
        ]
        ...
        return {
            "analysis_summary": {
                ...
                "average_potential_return": np.mean(
                    [
                        r["potential_return"]
                        for r in recommendations
                        if isinstance(r, dict)
    ...
    def _create_recommendation(self, opportunity: Dict) -> Optional[Dict]:
        ...
        risk_level = (
            "LOW" if confidence > 85 else "MODERATE" if confidence > 75 else "HIGH"
        )
    ...
    def _generate_reasons(self, opportunity: Dict) -> List[str]:
        ...
        if opportunity["overall_score"] > 80:
            reasons.append(
                f"Score global excellent: {opportunity['overall_score']:.1f}/100"
            )
...
class PortfolioOptimizer:
    ...
    async def suggest_portfolio(self, request: PortfolioRequest) -> Dict:
        ...
        investment_request = InvestmentRequest(
            min_confidence_score=70.0, max_recommendations=request.max_positions * 2
        )
        ...
        all_recs = (
            recommendations["strong_buy_recommendations"]
    ...
    def _diversify_by_sector(
        self, recommendations: List[Dict], max_positions: int
    ) -> List[Dict]:
        ...
        for sector, stocks in sectors.items():
            selected.extend(
                sorted(stocks, key=lambda x: x["confidence_score"], reverse=True)[:2]
            )
    ...
    def _allocate_weights(
        self, stocks: List[Dict], request: PortfolioRequest
    ) -> List[Dict]:
    ...
    def _calculate_portfolio_metrics(self, allocations: List[Dict]) -> Dict:
        ...
        return {
            ...
            "sector_breakdown": {
                k: round(v * 100, 1) for k, v in sector_breakdown.items()
            },
        }
...
async def investment_recommendations_get(
    min_confidence_score: float = Query(
        default=70.0, description="Score de confiance minimum"
    ),
    ...
    preferred_horizon: str = Query(
        default="MEDIUM_TERM", description="Horizon préféré"
    ),
):
...
╚══════════════════════════════════════════════════════════════╝
    ...
    uvicorn.run("api:app", host="0.0.0.0", port=8005, reload=True, log_level="info")
